...
(function () {
  ...
  var SLOTS = [
    ...
    { k: "sale_pending", h: "Sale pending", t: "Sale pending", icon: "i-clock", tone: "violet" }
  ];
      ...
      return '<tr data-id="' + v.id + '" class="' + (S.sel[v.id] ? 'sel' : '') + '" tabindex="0">' + COLS.map(function (c) {
        switch (c.k) {
          case 'ck': return '<td class="ckc"><input class="ck" type="checkbox" data-sel="' + v.id + '"' + (S.sel[v.id] ? ' checked' : '') + ' aria-label="Select ' + esc(v.stock) + '"></td>';
                              case "veh": return "<td><span class=\"obj\">" + (v.thumb ? "<img class=\"obj__th\" alt=\"\" loading=\"lazy\" src=\"img/" + esc(v.thumb) + "\">" : "<span class=\"obj__th obj__th--none\" title=\"No photos uploaded\">" + ic("i-cam-off") + "</span>") + "<span class=\"obj__b\"><span class=\"obj__n\" title=\"" + v.year + " " + esc(v.make) + " " + esc(v.model) + "\"><em>" + v.year + "</em>" + esc(v.make) + " " + esc(v.model) + "</span><span class=\"obj__m\"><a class=\"idtag\" href=\"#\" title=\"Stock number\" data-stop>" + esc(v.stock) + "</a>" + (v.lock ? "<span class=\"lockb\" title=\"" + esc(v.lock) + "\">" + ic("i-lock") + "</span>" : "") + "<span class=\"obj__vin\"><span>VIN</span>" + esc(v.vin) + "</span></span></span></span></td>";
