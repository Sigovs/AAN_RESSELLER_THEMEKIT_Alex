/* Gen 11 — bento intelligence on top, vehicle opens INLINE under its row (accordion sheet), Dock = Filters only.
...
(function () {
  ...
  (function () {
    ...
    function onScroll() {
      ...
      if (hd) { var h = window.scrollY > 0 && hd.getBoundingClientRect().top <= 114.5; if (h !== hdStuck) { hdStuck = h; hd.classList.toggle('hd--stuck', h); } var ex = document.getElementById('exp'); var veil = false; if (h && ex) { var er = ex.getBoundingClientRect(), hb = 114 + 48; veil = er.top < hb && er.bottom > 114; } hd.classList.toggle('hd--veil', veil); }
    }
