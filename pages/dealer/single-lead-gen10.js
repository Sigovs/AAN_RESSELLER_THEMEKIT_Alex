...
(function () {
  ...
  function seedActs() {
    ...
    if (L.act) acts.push({ d: L.act.d + 'T10:20', a: L.act.k, n: L.act.k === 'Comments Added' ? 'Called back, wants photos of the interior and the service records.' : L.act.k === 'Email Sent' ? '<subject>' + (L.car || 'Your enquiry') + '</subject>' : L.act.k === 'Status Changed' ? 'New Lead → ' + L.status : '', r: who });
  ...
  function render() {
    ...
    $('#ident').innerHTML = '<span class="mono"><b>' + esc(L.email) + '</b><button type="button" data-copy="' + esc(L.email) + '" title="Copy">' + ic('i-file') + '</button></span>' +
      (L.phone ? '<span class="mono"><b>' + esc(L.phone) + '</b><button type="button" data-copy="' + esc(L.phone) + '" title="Copy">' + ic('i-file') + '</button></span>' : '<span>no phone</span>') +
      '<span>' + esc(L.src) + '</span><span class="mono">' + esc(L.type) + '</span><span>created <b>' + fdt(L.created) + '</b></span>';
    // vehicle inset
    $('#vcar').className = 'agemix vcar' + (L.car ? '' : ' vcar--none');
    $('#vcar').innerHTML = L.car ? '<div class="vcar__img">' + (car && car.t ? '<img src="img/' + esc(car.t) + '" alt="">' : ic('i-cam-off')) + '<span class="st' + (car && car.sold ? ' st--sold' : '') + '">' + (car && car.sold ? 'Sold' : 'Available') + '</span></div><div class="vcar__t" title="' + esc(L.car) + '">' + esc(L.car) + '</div><div class="vcar__m">' + (L.stock ? 'Stock ' + esc(L.stock) : 'no stock #') + (car ? ' · VIN ' + esc(car.vin) : '') + '</div><div class="vcar__acts"><a class="p" href="all-vehicles-gen10.html">Open in Inventory →</a><button type="button" data-act="change-veh">Change vehicle…</button></div>'
      : '<div class="vcar__img">' + ic('i-cam-off') + '</div><div class="vcar__t">No vehicle of interest</div><div class="vcar__m">attach one from the inventory</div><div class="vcar__acts"><button type="button" class="p" data-act="change-veh">Search Inventory</button></div>';
    // workflow module
    $('#wf-kv').innerHTML = '<div><div class="v">' + esc(L.status) + '</div><div class="l">status</div></div><div><div class="v' + (L.rep === 'Unassigned' ? ' warn' : '') + '" title="' + esc(L.rep) + '">' + esc(L.rep) + '</div><div class="l">assigned to</div></div><div><div class="v' + (L.pastDue ? ' due' : '') + '">' + (L.follow ? fd(L.follow) : '—') + '</div><div class="l">follow-up' + (L.pastDue ? ' · past due' : '') + '</div></div>';
    // timeline module (activity · emails · notes · last email · modified · newsletter)
    var lastEmail = emails.length ? emails[0].d.slice(0, 5) : '—', modified = acts.length ? acts[0].d.slice(0, 5) : '—';
    var tl = [[acts.length, 'Activity', 'entries', 'signal'], [emails.length, 'Emails', 'in + out', 'violet'], [L.notes, 'Notes', 'comments added', 'slate'], [lastEmail, 'Last email', emails.length ? emails[0].dir === 'IN' ? 'from the buyer' : 'to the buyer' : 'none yet', 'violet'], [modified, 'Modified', 'last change', 'slate'], [L.newMail ? 'Yes' : 'No', 'New mail', L.newMail ? 'unread reply' : 'nothing unread', L.newMail ? 'danger' : 'slate']];
    $('#tl-g').innerHTML = tl.map(function (t) { return '<div class="ar ar--' + t[3] + '"><span class="ar__n">' + esc(t[0]) + '</span><span class="ar__b"><span class="ar__l"><span class="full">' + t[1] + '</span><span class="short">' + t[1] + '</span></span><span class="ar__m"><i style="--w:' + (typeof t[0] === 'number' ? Math.min(100, t[0] * 12) : 100) + '%"></i></span></span><span class="ar__p">' + esc(t[2]) + '</span></div>'; }).join('');
    $('#tl-s').textContent = 'created ' + fd(L.created) + ' · modified ' + modified + ' · last email ' + lastEmail;
    // section counts
    ...
    // activity table
    $('#act-tbl').innerHTML = '<table><thead><tr><th>Date</th><th>Action</th><th>Notes</th><th>Rep</th></tr></thead><tbody>' + (acts.length ? acts.map(function (a) { return '<tr><td class="d">' + esc(a.d) + '</td><td class="a"><b>' + esc(a.a) + '</b></td><td class="n">' + esc(a.n) + '</td><td class="r">' + esc(a.r) + '</td></tr>'; }).join('') : '<tr><td class="empty" colspan="4">No activity on this lead yet.</td></tr>') + '</tbody></table>';
    ...
    // emails table
    $('#em-tbl').innerHTML = '<table><thead><tr><th></th><th>Date</th><th>Subject</th><th>From / To</th></tr></thead><tbody>' + (emails.length ? emails.map(function (e) { return '<tr><td><span class="dir dir--' + e.dir.toLowerCase() + '">' + e.dir + '</span></td><td class="d">' + esc(e.d) + '</td><td class="a"><b>' + esc(e.s) + '</b></td><td class="r">' + esc(e.w) + '</td></tr>'; }).join('') : '<tr><td class="empty" colspan="4">No emails on this lead.</td></tr>') + '</tbody></table>';
    // dock: workflow + facts
    ...
    $('#facts').innerHTML = [['Source', L.src], ['Type', L.type, 1], ['Vendor', window.LD.totals.dealer], ['Provider', L.provider], ['Created', fdt(L.created)], ['Modified', modified === '—' ? '—' : acts[0].d], ['Last email', lastEmail === '—' ? '—' : emails[0].d], ['IP', (70 + idx % 30) + '.' + (69 + idx) + '.13.' + (16 + idx), 1]].map(function (f) { return '<div><dt>' + f[0] + '</dt><dd' + (f[2] ? ' class="mono"' : '') + '>' + esc(f[1]) + '</dd></div>'; }).join('');
    $('#nl').checked = idx % 3 === 0;
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    if ((el = t.closest('[data-sec]'))) { e.preventDefault(); var s = $('#' + el.dataset.sec); if (s) window.scrollTo({ top: s.offsetTop - 56 - 58 - 14, behavior: 'smooth' }); return; }
    ...
    if ((el = t.closest('[data-act="note"]'))) { $('#note-in').focus(); window.scrollTo({ top: $('#s-activity').offsetTop - 56 - 58 - 14, behavior: 'smooth' }); return; }
    if ((el = t.closest('[data-act="email"]'))) { window.scrollTo({ top: $('#s-emails').offsetTop - 56 - 58 - 14, behavior: 'smooth' }); return; }
